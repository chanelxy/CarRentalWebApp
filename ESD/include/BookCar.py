...
def bookcar():
    ...
    if not available:
        return message, 400
         ...
         #retrieve crID
        carRenterResponse = requests.get('http://localhost:7200/carrenter/' + crUsername)
        ...
        carOwnerResponse = requests.get('http://localhost:7100/carowner/' + car['coUsername'])
        ...
        for i in range(delta.days + 1):
            ...
            requests.post('http://localhost:7300/rentaltransactions', json = row_data)
        return jsonify({"message": "rental_transactions_inserted"}), 200

def verify_car_available(cID, startDate, endDate):
    response = requests.get('http://localhost:7300/rentaltransactions/' + str(cID) + "/" + startDate + "/" + endDate)
...
if __name__ == '__main__': #this allows us to run flask app without explicitly using python -m flask run. Can just run python filename.py in terminal
    app.run(host='0.0.0.0',port=8100, debug=True) #need to use different port for each microservice. By default, it is 5000. Project need to use diff port no.s