...
@app.route("/searchcar")
def get_all_cars():
    carResponse = requests.get('http://localhost:7000/car')
    return json.loads(carResponse.content)

@app.route("/searchcar/<int:cID>")
def get_car_by_cID(cID):
    carResponse = requests.get('http://localhost:7000/car/cid/' + str(cID))
    return json.loads(carResponse.content)
...
def get_car_filter_options(start_date, end_date, seating_capacity, transmissiontype, postalcode):
    if transmissiontype == "both":
        carResponse = requests.get('http://localhost:7000/car/capacity/' + str(seating_capacity))
        json.loads(carResponse.text) 
        carList = json.loads(carResponse.text)['car']
    else:
        carResponse = requests.get('http://localhost:7000/car/capacity/' + str(seating_capacity) + "/" + transmissiontype)
        json.loads(carResponse.text)
        carList = json.loads(carResponse.text)['car']
    
    rentalTransactionResponse = requests.get('http://localhost:7300/rentaltransactions/' + start_date + "/" + end_date)
    ...
    for car in carList:
        if car['cID'] not in carIDSet:
            # coordinatesResult = requests.get("http://localhost:9000/getcoordinates/" + str(car['postalCode']))
            # if coordinatesResult.status_code == 400:
            #     coordinates = {"lat": 0, "lng": 0}
            # else:
            #     coordinates = json.loads(coordinatesResult.text)['coordinates']
            distanceResult = requests.get("http://localhost:9000/getdistance/" + postalcode + "/" + str(car['postalCode']))
