...
def find_by_crID(crID):
    transaction = {"RentalTransaction": [crID.json() for crID in RentalTransaction.query.filter_by(crID=crID).all()]}
...
def find_by_coID(coID):
    transaction = {"RentalTransaction": [coID.json() for coID in RentalTransaction.query.filter_by(coID=coID).all()]}
    if transaction != {"RentalTransaction": []}:
        return jsonify(transaction)
    return jsonify({"message": "No transactions found."}), 404  
