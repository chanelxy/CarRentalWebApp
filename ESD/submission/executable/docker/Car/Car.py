...
def get_car_by_cousername(coUsername):
    car = {"car": [car.json() for car in Car.query.filter_by(coUsername=coUsername).all()]}
    if car != {"car": []}:
        return jsonify(car)
    return jsonify({"message": "Car not found."}), 404
...
@app.route("/car/capacity/<string:capacity>/<string:transmissiontype>")
def get_car_by_capacity_and_transmissiontype(capacity, transmissiontype):
    result = Car.query.filter(Car.transmissionType==transmissiontype).filter(Car.capacity==capacity)
